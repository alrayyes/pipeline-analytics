...
- [ ] 1.1 Add `limit`/`offset`/`repoId`/`forge` query params and a
...
- [ ] 2.1 Add `metrics.PipelineListFilter{RepoID, Forge, Limit, Offset}`
...
- [ ] 2.2 Update `internal/metrics/sqlite/store.go`'s `ListPipelines`: add
...
- [ ] 2.3 Update `metrics.Service.ListPipelines` to accept
...
- [ ] 3.1 Add `pipelineListDTO{Pipelines []pipelineSummaryDTO, HasMore
...
- [ ] 4.1 Update `web/src/routes/+page.svelte`'s `PipelineSummary[]`
...
- [ ] 4.2 Move the forge filter and repo selector into the `/api/pipelines`
      ...
      scoping decision).
- [ ] 4.3 Update the "Showing N of M" summary text to describe the
