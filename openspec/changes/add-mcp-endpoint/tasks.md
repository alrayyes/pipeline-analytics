...
- [ ] 1.1 Add `github.com/modelcontextprotocol/go-sdk` at its latest
...
- [ ] 1.2 Add `github.com/getkin/kin-openapi` (test-only use) at its
...
- [ ] 2.2 Run `bunx @redocly/cli lint openapi/openapi.yaml` and verify
      it's clean
...
- [ ] 4.1 Write a failing case in `mcp_test.go`: calling the
...
- [ ] 4.2 Implement the `list_pipelines` tool in `mcp.go`, calling
      `deps.Metrics` the same way `pipelinesHandler.list` does; verify
      4.1 passes
- [ ] 4.3 Write a failing case: calling the `get_pipeline` tool for a
...
- [ ] 4.4 Implement `get_pipeline`; verify 4.3 passes
- [ ] 4.5 Write a failing case: calling `list_pipeline_steps` for a
...
- [ ] 4.6 Implement `list_pipeline_steps`; verify 4.5 passes
- [ ] 4.7 Write a failing case: calling `list_pipeline_flaky_runs`
      returns the same runs `GET
      /api/pipelines/{pipelineId}/flaky-runs` returns
- [ ] 4.8 Implement `list_pipeline_flaky_runs`; verify 4.7 passes

## 5. Usage and cross-pipeline tools

- [ ] 5.1 Write a failing case: calling `get_repo_usage` for a tracked
      repo ID returns the same runner-minutes figures `GET
      /api/repos/{repoId}/usage` returns
- [ ] 5.2 Implement `get_repo_usage`; verify 5.1 passes
- [ ] 5.3 Write a failing case: calling `list_unhealthy_steps` returns
      the same steps `GET /api/steps/unhealthy` returns
- [ ] 5.4 Implement `list_unhealthy_steps`; verify 5.3 passes
- [ ] 5.5 Write a failing case: the tool list from `tools/list`
      contains exactly these six tools and none of them accepts a
      write/mutating call
- [ ] 5.6 Verify 5.5 passes against the implementation from sections 4
      and 5
