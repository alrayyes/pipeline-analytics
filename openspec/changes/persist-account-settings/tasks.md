...
- [ ] 1.1 Add the `account_settings` migration (`user_id` PK referencing
      `webauthn_users`, `data TEXT`, `updated_at`) and verify it applies
      cleanly via the existing migration test harness
- [ ] 1.2 Add a settings store (get/patch, with `null`-clears-to-default
...
- [ ] 3.1 Add a `/settings` route with the theme control (light/dark/
      system) and remove the theme toggle from `Nav.svelte`
- [ ] 3.2 Add a "Reset filters" control to the Pipelines page that
...
- [ ] 3.3 Run the Playwright suite (including the axe-core scan on the
